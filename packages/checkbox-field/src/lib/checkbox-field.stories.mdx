...
<Meta title="Components/CheckboxField" component={CheckboxField} />

# CheckboxField <img src={`https://img.shields.io/badge/Version-v${pkg.version}-brightgreenn`} />

### Installation

First install the latest version of the package from `@novatics-ui/checkbox-field`.

```shell
  yarn add -D @novatics-ui/checkbox-field
```

```shell
  npm install --dev @novatics-ui/checkbox-field
```

and use it

```tsx
import ... from '@novatics-ui/checkbox-field';
