...
<Meta title="Components/Masked Textfield" component={MaskedTextField} />
...
First install the latest version of the package from `@novatics-ui/masked-text-field`.

```bash
yarn add @novatics-ui/masked-text-field
```
...
It is possible to use the maskPreset attribute to use one of the [pre-configured masks](https://github.com/Novatics/novatics-ui/blob/master/packages/masked-text-field/src/lib/mask-presets.ts).
